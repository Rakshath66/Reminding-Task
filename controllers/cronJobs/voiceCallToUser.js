...
const voiceCallToUser = async () => {
    try {
      ...
      const overdueTasks = await Task.find({
        ...
        status: { $ne: 'DONE' },
      }).sort({ priority: 1 });  // Sort by priority ascending // Populate the userId field to retrieve associated user data
      ...
      for (const task of overdueTasks) {
        ...
        // Check if the user has a valid phone number // Call the user using Twilio
        if (user && user.phone_number) {
          await twilioHelper.sendVoiceCall("+91"+user.phone_number, `Task overdue: ${task.title}. plz complete it`);
...
module.exports = {voiceCallToUser};
